...
## News Signals + Portfolio Exposure

**What it answers:** "Which news events require advisor action, and which clients/accounts are sitting in those positions right now?"  
**Genie data sources:** `gold_unified_signals` (filter `source_type = 'news'`), `holdings`, `clients`, `accounts`, `bronze_company_profiles`

> **Note for Genie:** News signals are stored in `gold_unified_signals` with `source_type = 'news'`. There is no separate `gold_news_signals` table — always filter on `source_type = 'news'` when the question is specifically about news. `source_description` holds the article headline. `signal_type` is a free-form label generated by Claude (e.g., "Non-Accrual Designation", "CEO Departure", "Q3 Earnings Miss") rather than a fixed vocabulary.

---

### [GENIE] "Which action-needed news signals in the last 60 days have exposure in my book?"

Returns one row per (signal × ticker), showing how many clients and accounts hold the affected name and total dollar exposure across the book. Use this as the morning alert feed.
...
SELECT
  ...
  g.sentiment,
  g.severity_score,
  ROUND(SUM(h.market_value), 2)          AS total_book_exposure,
  COUNT(DISTINCT a.account_id)            AS accounts_exposed,
  COUNT(DISTINCT c.client_id)             AS clients_exposed,
  LEFT(g.source_description, 80)          AS headline,
  LEFT(g.rationale, 200)                  AS rationale
FROM ahtsa.awm.gold_unified_signals g
JOIN ahtsa.awm.holdings          h  ON h.ticker     = g.symbol
JOIN ahtsa.awm.accounts          a  ON a.account_id = h.account_id
JOIN ahtsa.awm.clients           c  ON c.client_id  = a.client_id
LEFT JOIN ahtsa.awm.bronze_company_profiles cp ON cp.symbol = g.symbol
WHERE g.source_type           = 'news'
  AND g.advisor_action_needed = true
  AND g.signal_date           >= CURRENT_DATE() - INTERVAL 60 DAYS
GROUP BY ALL
ORDER BY g.severity_score DESC, total_book_exposure DESC
```

> **Tested result:** Returns signals across AAPL, XLK, XLY, AMZN, MSFT, META, GOOGL, NVDA, and others. XLK appears across 213 accounts / 156 clients due to ETF look-through. High-severity signals dominate (severity_score = 0.9).

---

### [GENIE] "Which clients have the most dollar exposure to tickers with action-needed news?"

Client-priority ranked list for outreach — shows which UHNW clients need to be called first based on flagged position size. One row per client, aggregated across all action-needed news tickers they hold.
...
SELECT
  ...
  c.tier,
  c.risk_profile,
  COUNT(DISTINCT g.symbol)                                    AS flagged_tickers,
  COUNT(DISTINCT g.signal_id)                                 AS total_news_signals,
  MAX(g.severity_score)                                       AS max_severity,
  ROUND(SUM(h.market_value), 2)                               AS total_flagged_exposure,
  ROUND(
    SUM(h.market_value)
    / NULLIF(SUM(SUM(h.market_value)) OVER (PARTITION BY c.client_id), 0) * 100,
  2)                                                          AS pct_of_portfolio,
  ARRAY_JOIN(ARRAY_SORT(COLLECT_SET(g.symbol)), ', ')         AS tickers
FROM ahtsa.awm.gold_unified_signals g
JOIN ahtsa.awm.holdings          h  ON h.ticker     = g.symbol
JOIN ahtsa.awm.accounts          a  ON a.account_id = h.account_id
JOIN ahtsa.awm.clients           c  ON c.client_id  = a.client_id
WHERE g.source_type           = 'news'
  AND g.advisor_action_needed = true
  AND g.signal_date           >= CURRENT_DATE() - INTERVAL 60 DAYS
GROUP BY c.advisor_id, c.client_id, c.client_name, c.tier, c.risk_profile
ORDER BY total_flagged_exposure DESC
LIMIT 30
```

> **Tested result:** Returns 20+ clients. Top client has exposure across 12 flagged tickers (99 signals), with total flagged position value in the hundreds of millions. `tickers` column shows the specific names: AAPL, ADBE, AMZN, META, MSFT, NVDA, XLK, etc.

---

### [GENIE] "Show me the account-level position detail for high-severity action-needed news from the last 30 days"

The most actionable view — drills to individual account positions so the advisor can see exactly which accounts need a call, the position size, and the specific headline that triggered the flag.
...
SELECT
  ...
  g.signal_value,
  g.sentiment,
  ROUND(h.market_value, 2)                                    AS position_value,
  ROUND(
    h.market_value
    / NULLIF(SUM(h.market_value) OVER (PARTITION BY a.account_id), 0) * 100,
  2)                                                          AS pct_of_account,
  ROUND(h.unrealized_gl, 2)                                   AS unrealized_gl,
  LEFT(g.source_description, 100)                             AS headline,
  LEFT(g.rationale, 250)                                      AS rationale
FROM ahtsa.awm.gold_unified_signals g
JOIN ahtsa.awm.holdings          h  ON h.ticker     = g.symbol
JOIN ahtsa.awm.accounts          a  ON a.account_id = h.account_id
JOIN ahtsa.awm.clients           c  ON c.client_id  = a.client_id
...
WHERE g.source_type           = 'news'
  AND g.advisor_action_needed = true
  AND g.signal_value          = 'High'
  AND g.signal_date           >= CURRENT_DATE() - INTERVAL 30 DAYS
ORDER BY g.signal_date DESC, h.market_value DESC
LIMIT 50
```

> **Tested result:** Returns 25 rows across XLK, XLY, AAPL, AMZN, MSFT, META, GOOGL, NVDA positions. Includes `unrealized_gl` so the advisor can see whether a flagged position also has a tax-loss harvesting opportunity. `pct_of_account` shows how concentrated the exposure is — a 15% position in a flagged name is more urgent than a 2% position.
